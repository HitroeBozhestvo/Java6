...
public class IntegralClient {
    ...
    private PrintWriter out; // поток отправки данных серверу
    private int tasksForThisClient; // число задач отдельного клиента
    ...
    public static void main(String[] args) {
        try (Socket socket = new Socket(SERVER_ADDRESS, SERVER_PORT);
            ...
            while (true) {
                ...
                client.computeIntegral(a, b, dx, numClients, clientId); // передача параметров для расчёта
            }
        } catch (IOException e) {
    ...
    private void computeIntegral(double a, double b, double dx, int numClients, int clientId) {
        ...
        int extraTasks = NUM_THREADS % numClients; // распределение остатка
        ...
        if (startTask >= NUM_THREADS) {
            System.out.println("Клиент " + clientId + ": задач не назначено"); // если клиентов больше чем потоков
            return;
        }
